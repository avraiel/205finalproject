...
with psycopg.connect("dbname=vectordb user=postgres", password=os.getenv('PSQL_PASSWORD')) as conn:
    with conn.cursor() as cur:
        cur.execute("""
                CREATE TABLE IF NOT EXISTS openailite (
                    ...
                    author varchar(255))
                """)
        print("Created the openai table!")
        for entry in dataset_clean:
            ...
            for sentence in response.data:
                ...
                cur.execute("""
                    INSERT INTO openailite (embedding, raw, title, website, category, url, date, author)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", 
                            (list(embedding), raw, title, website, category, url, date, author)
                    )
                chunk_idx += 1
