...
model = "dost-asti/RoBERTa-tl-cased"
total_articles = 0
total_entries = 0
...
with psycopg.connect("dbname=vectordb user=postgres", password=os.getenv('PSQL_PASSWORD')) as conn:
    with conn.cursor() as cur:
        cur.execute("""
                CREATE TABLE IF NOT EXISTS dostlite (
        ...
        for entry in dataset_clean:
            ...
            for sentence in sentence_embeddings:
                ...
                cur.execute("""
                    INSERT INTO dostlite (embedding, raw, title, website, category, url, date, author)
